...
class ContextExtraction(nn.Module):
    ...
    def __init__(self, block, num_blocks,dense = True,dilation=True, inplanes = 16):
        ...
        self.block1 = nn.Sequential(*[
            nn.Conv2d(3, self.inplanes, kernel_size=7, stride=1, padding=3, bias=False),
            nn.ReLU(inplace=True)
        ])

        self.dilation = dilation
        # for i in range(1, num_blocks):
        self.block2 = block(self.inplanes, self.inplanes, dilation = 4 if dilation else 1) if num_blocks>=2 else None
        self.block3 = block(self.inplanes, self.inplanes, dilation = 8 if dilation else 1) if num_blocks>=3 else None
        self.block4 = block(self.inplanes, self.inplanes, dilation = 16 if dilation else 1) if num_blocks>=4 else None
...
#     return model
# def S2DF_3dense():
#     model = ContextExtraction(BasicBlock,3,dense=True)
#     return model
# def S2DF_3last():
#     model = ContextExtraction(BasicBlock,3,dense=False)
#     return model
# def S2DF_2dense():
#     model = ContextExtraction(BasicBlock,2,dense=True)
#     return model
