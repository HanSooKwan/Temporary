...
class DepthAwareFlowInitialization(nn.Module):
    ...
    def forward(self, flow, inv_depth):
        return DepthAwareFlowInitialization_Function.apply(flow, inv_depth, flow.requires_grad, self.backward,
                                                           self.device)
...
class DepthAwareFlowInitialization_Function(Function):
    ...
    def forward(ctx, flow, inv_depth, requires_grad, is_backward, device):
        ...
        # flow shape: B, 2, H, W    /    inv_depth shape: B, 1, H, W (Unsqueeze it)
        ...
        in_range = torch.stack(
            [mesh_after[:, 0] >= 0, mesh_after[:, 0] < W, mesh_after[:, 1] >= 0, mesh_after[:, 1] < H], dim=0)
        ...
        if B == 1:
            mesh_after_raveled = torch.reshape(mesh_after[:, 0] + mesh_after[:, 1] * W,
                                               [unravel_length])  # Change to 1D
        else:
            mesh_after_raveled = torch.reshape(
                mesh_after[:, 0] + mesh_after[:, 1] * W + torch.arange(start=0, end=B).unsqueeze(dim=-1).unsqueeze(
                    dim=-1) * H * W, [unravel_length])  # Change to 1D
    ...
    def backward(ctx, grad_depth_aware_flow):
        ...
        if ctx.needs_input_grad[0]:
            ...
            grad_flow = grad_depth_aware_flow[:, mesh_after_raveled] * inv_sum_weight[:,
                                                                       mesh_after_raveled] * weights_raveled.unsqueeze(
                dim=0)
        ...
        if ctx.needs_input_grad[1]:
            ...
            grad_inv_depth = (flow_raveled - holes_unfilled[:, mesh_after_raveled]) * inv_sum_weight[:,
                                                                                      mesh_after_raveled] * grad_depth_aware_flow[
                                                                                                            :,
                                                                                                            mesh_after_raveled] * (
                                         weights_raveled.unsqueeze(dim=0) != 0)
